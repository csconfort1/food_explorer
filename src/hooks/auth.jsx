...
function AuthProvider({children}){
    ...
    function authenticateUser({ user,token}) {setUserInfos(user);
      ...
      function validateSessionTime(mostRecentLoginTime) {
        ...
        const timedOutOfSession = currentTime - mostRecentLoginTime > maximumTimeLogged;
      ...
      function getSavedData() {
        ...
        const session = JSON.parse(localStorage.getItem('@food_explorer:session_created_at'));
    
        return { user, token, session };
      }
    
      function includeSavedData(user, token) {setUserInfos(user);
        api.defaults.headers.common['Authorization'] = `Bearer ${token}`;
      }
      ...
      useEffect(() => {
        ...
        if (wasTheUserAlreadyLoggedIn) {loadLastSessionData({user,token,session});
        }
      }, []);
    
      return (
        <AuthContext.Provider value={{ userInfos, authenticateUser, clearLoginData }}>
          {children}
